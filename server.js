...
class GameState extends colyseus.Room {
    ...
    onCreate(options) {
        // Store room options (songId, private, etc.)
        this.roomOptions = options || {};
        this.songId = options.songId || 1;
        this.isPrivate = options.private || false;
        
        this.clock.setInterval(() => this.updateGameState(), 1000 / 60);
        
        console.log(`Room created: ${this.roomId}, Song: ${this.songId}, Private: ${this.isPrivate}`);
    }
...
// Define game room with metadata handling
gameServer.define('rhythm_game', GameState, {
    maxClients: 2,
    // Enable room listing for lobby functionality
    listing: {
        // Custom filter for lobby list
        filter: (room) => {
            const options = room.roomOptions || {};
            // Don't list private rooms in the public lobby
