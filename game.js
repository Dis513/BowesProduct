...
class NeonNightmare {
    ...
    async initializeFirebase() {
        ...
        const tryInit = () => {
            if (window.firebaseAuth) {
                ...
                console.log('Firebase initialized successfully:', { auth: !!this.auth, db: !!this.db, provider: !!this.provider });
                ...
                if (this.authBtn) {
                    ...
                    console.log('Auth button handler set up successfully');
                } else {
                    console.error('Auth button not found!');
                }
            } else {
                console.log(`Firebase not yet initialized, retrying... (${retries + 1}/${maxRetries})`);
                retries++;
    ...
    async updateProfileDisplay(user) {
        ...
        // Update level and XP display after profile is loaded
    ...
    setupEventListeners() {
        ...
        // Event delegation for menu buttons - more robust approach
        document.addEventListener('click', (e) => {
            ...
            switch (target.id) {
                ...
                case 'setDifficultyHard':
                    ...
                    break;
                case 'setDifficultyExpert':
                    this.setDifficulty('expert');
                    break;
                case 'setDifficultyMaster':
                ...
                case 'uploadNewSong':
                    ...
                    break;
                case 'selectLevelFromComplete':
                    this.showLevelSelect();
                    break;
                case 'mainMenuFromComplete':
                    this.returnToMenu();
                    break;
                case 'restartFromGameOver':
                    this.restartSong();
                    break;
                case 'uploadFromGameOver':
                    this.returnToMenu();
                    const uploadBtnGameOver = document.getElementById('uploadButton');
                    if (uploadBtnGameOver) uploadBtnGameOver.click();
                    break;
                case 'mainMenuFromGameOver':
                    this.returnToMenu();
                    break;
                case 'showMultiplayer':
                    console.log('Multiplayer clicked');
                    if (this.multiplayerManager) {
                        this.multiplayerManager.showMultiplayerMenu();
                    }
                    break;
            }
        }, true);
        
        console.log('Event delegation set up for menu buttons');
        
        // Also keep individual listeners as backup
        const selectLevelBtn = document.getElementById('selectLevel');
        console.log('Select Level button found:', !!selectLevelBtn);
        if (selectLevelBtn) {
            selectLevelBtn.addEventListener('click', () => {
                console.log('Select Level clicked (backup listener)');
                this.showLevelSelect();
            });
        ...
        console.log('Open Settings button found:', !!openSettingsBtn);
        if (openSettingsBtn) {
            openSettingsBtn.addEventListener('click', () => {
                console.log('Settings clicked (backup listener)');
                this.showSettings();
            });
        ...
        console.log('View Leaderboards button found:', !!viewLeaderboardsBtn);
        if (viewLeaderboardsBtn) {
            viewLeaderboardsBtn.addEventListener('click', () => {
                console.log('Leaderboards clicked (backup listener)');
                this.showLeaderboards();
            });
        ...
        this.frets.forEach((fret, index) => {
            ...
            if (button) {
                ...
                button.addEventListener('mousedown', () => {
                    console.log(`Mouse down on ${fret} button`);
                ...
                button.addEventListener('touchend', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log(`Touch end on ${fret} button`);
                ...
                console.log(`Attached event listeners to ${fret} button`);
            }
        });
    ...
    async loadLevelAudio(level) {
        ...
        try {
            ...
            this.hideLoadingScreen();
            if (level === 1) {
                alert(`Error: Level 1 file not found.\n\nPlease choose another level, coming soon.`);
            } else {
    ...
    playAudio() {
        ...
        // Only connect to destination (speakers) if NOT multiplayer
        // In multiplayer, each player plays their own audio locally
    ...
    handleFretPress(fret) {
        // Only check game state if we're not in menu mode
        if (!this.isPlaying || this.isPaused) {
            console.log(`Fret ${fret} pressed but game not active (isPlaying: ${this.isPlaying}, isPaused: ${this.isPaused})`);
            return;
        }
        ...
        // Animate fret button - always animate even if no note hit
        this.animateFretButton(fret);
    }
    ...
    startHoldNote(note, fret) {
        const currentTime = this.audioContext.currentTime - this.startTime;
        const holdDuration = 2.0; // 2 second hold duration for better gameplay
    ...
    endHoldNote(fret) {
        if (this.activeHolds[fret]) {
            ...
            const minimumHold = hold.holdDuration * 0.5; // Must hold for at least 50% of duration
            
            // Clear the scoring interval
            clearInterval(hold.scoreInterval);
            
            // Check if held for at least minimum required time
            if (holdTime < minimumHold) {
                // Released too early - count as miss
                console.log(`Hold note released too early: ${holdTime.toFixed(2)}s < ${minimumHold.toFixed(2)}s`);
                this.combo = 0;
                this.multiplier = 1;
                this.misses++;
                this.drainHealth();
                ...
                // Successfully held for at least minimum required time
                console.log(`Hold note successful: ${holdTime.toFixed(2)}s >= ${minimumHold.toFixed(2)}s`);
            }
    ...
    updateHUD() {
        ...
        // Send score update to multiplayer server
    ...
    cleanup() {
        ...
        // Send multiplayer game finished if multiplayer
        if (this.isMultiplayer && this.multiplayerManager) {
            this.multiplayerManager.sendGameFinished(this.score);
        }
    }
    ...
    selectTheme(themeName) {
        ...
        if (themeName === 'rainbow-six-siege') {
            ...
            if (highScore < 1000) {
                alert('🔒 Siege Ops theme is locked! Score 1,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'star-wars') {
            const highScore = this.getHighScore();
            if (highScore < 10000) {
                alert('🔒 Galactic Wars theme is locked! Score 10,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'sonic') {
            const highScore = this.getHighScore();
            if (highScore < 50000) {
                alert('🔒 Speed Demon theme is locked! Score 50,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'dark-souls') {
            const highScore = this.getHighScore();
            if (highScore < 100000) {
                alert('🔒 Souls Nightmare theme is locked! Score 100,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'warcraft') {
            const highScore = this.getHighScore();
            if (highScore < 500000) {
                alert('🔒 Realms of War theme is locked! Score 500,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'avatar') {
            ...
            if (highScore < 1000000) {
                alert('🔒 Pandora theme is locked! Score 1,000,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'final-fantasy') {
            ...
            if (highScore < 5000000) {
                alert('🔒 Crystal Chronicles theme is locked! Score 5,000,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'call-of-duty') {
            const highScore = this.getHighScore();
            if (highScore < 5000000) {
                alert('🔒 Modern Warfare theme is locked! Score 5,000,000 points to unlock it.');
                return;
            }
        }
        ...
        if (themeName === 'minecraft') {
            const highScore = this.getHighScore();
            if (highScore < 5000000) {
                alert('🔒 Block Wars theme is locked! Score 5,000,000 points to unlock it.');
                return;
            }
        }
  ...
  getHighScore() {
    ...
    updateThemeUnlockStatus() {
        ...
        if (rainbowSixSiegeTheme) {
            ...
            if (highScore >= 1000) {
                ...
                if (themeInfo) themeInfo.textContent = `🔒 Unlock at 1,000 points (${(1000 - highScore).toLocaleString()} more)`;
                if (btn) {
                    btn.disabled = true;
        ...
        if (starWarsTheme) {
            ...
            if (highScore >= 10000) {
                ...
                allUnlocked = false;
                if (progressTarget > 10000) progressTarget = 10000;
                starWarsTheme.classList.add('locked');
                if (themeInfo) themeInfo.textContent = `🔒 Unlock at 10,000 points (${(10000 - highScore).toLocaleString()} more)`;
                if (btn) {
                    btn.disabled = true;
                    btn.textContent = '🔒 Locked';
                }
        ...
        if (sonicTheme) {
            ...
            if (highScore >= 50000) {
                ...
                allUnlocked = false;
                if (progressTarget > 50000) progressTarget = 50000;
                sonicTheme.classList.add('locked');
                if (themeInfo) themeInfo.textContent = `🔒 Unlock at 50,000 points (${(50000 - highScore).toLocaleString()} more)`;
                if (btn) {
                    btn.disabled = true;
                    btn.textContent = '🔒 Locked';
                }
        ...
        if (darkSoulsTheme) {
            ...
            if (highScore >= 100000) {
                ...
                allUnlocked = false;
                if (progressTarget > 100000) progressTarget = 100000;
        ...
        if (avatarTheme) {
            ...
            if (highScore >= 1000000) {
                ...
                allUnlocked = false;
                if (progressTarget > 1000000) progressTarget = 1000000;
                avatarTheme.classList.add('locked');
                if (themeInfo) themeInfo.textContent = `🔒 Unlock at 1,000,000 points (${(1000000 - highScore).toLocaleString()} more)`;
                if (btn) {
                    btn.disabled = true;
                    btn.textContent = '🔒 Locked';
                }
        ...
        if (finalFantasyTheme) {
            ...
            if (highScore >= 5000000) {
                ...
                allUnlocked = false;
                if (progressTarget > 5000000) progressTarget = 5000000;
                finalFantasyTheme.classList.add('locked');
                if (themeInfo) themeInfo.textContent = `🔒 Unlock at 5,000,000 points (${(5000000 - highScore).toLocaleString()} more)`;
                if (btn) {
                    btn.disabled = true;
                    btn.textContent = '🔒 Locked';
                }
    ...
    initializeMultiplayer() {
        // Initialize multiplayer manager after a short delay to ensure DOM is ready
        setTimeout(() => {
            try {
                if (typeof MultiplayerManager !== 'undefined') {
                    ...
                    console.warn('MultiplayerManager class not found');
                }
            } catch (error) {
