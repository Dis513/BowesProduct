...
class NeonNightmare {
    ...
    generateNotesFromAudio(channelData, sampleRate) {
        ...
        filteredBeats.forEach((beat, index) => {
            ...
            const isHold = false; // Hold notes disabled
            ...
            if (isSimultaneous) {
                ...
                notes.push({
                    ...
                    hit: false,
                    missed: false,
                    isHold: false, // Hold notes disabled
                    isSimultaneous: true
                });
            }
        });
    ...
    gameLoop() {
       ...
       updateNotes(currentTime) {
           this.activeNotes.forEach(note => {
               ...
               // Only check for miss if note has been spawned long enough to reach target line
               // Notes need at least 1.5 seconds to travel from spawn to target
               const timeSinceSpawn = currentTime - (note.spawnTime || 0);
        ...
        this.notes.forEach(note => {
            if (!note.hit && !note.missed && 
                ...
                note.time > currentTime) {
                   note.spawnTime = currentTime; // Track when note was spawned
