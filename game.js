...
class NeonNightmare {
    ...
    selectTheme(themeName) {
        ...
        if (themeName === 'dark-souls') {
            ...
            if (highScore < 1000) {
                alert('🔒 Dark Souls theme is locked! Score 1000 points to unlock it.');
                return;
            }
        }
    ...
    updateThemeUnlockStatus() {
        ...
        if (unlockScore) {
            unlockScore.textContent = highScore;
        }
        
        // Update progress bar
        if (progressFill) {
            const progress = Math.min((highScore / 1000) * 100, 100);
        ...
        if (darkSoulsTheme) {
            ...
            if (highScore >= 1000) {
                ...
                if (themeInfo) themeInfo.textContent = `🔒 Unlock at 1000 points (${1000 - highScore} more)`;
                if (btn) {
                    btn.disabled = true;
