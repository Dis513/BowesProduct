...
<div id="container">
  ...
  <div id="floatingToolbar">
    <div class="tool-group">
      <div class="tool-btn active" id="selectTool" title="Selection Tool (V)">⬚</div>
      <div class="tool-btn" id="directSelectTool" title="Direct Selection Tool (A)">↖</div>
      <div class="tool-btn" id="magicWandTool" title="Magic Wand Tool (Y)">✦</div>
    </div>
    <div class="tool-group">
      <div class="tool-btn" id="penTool" title="Pen Tool (P)">✒</div>
      <div class="tool-btn" id="pencilTool" title="Pencil Tool (N)">✎</div>
      <div class="tool-btn" id="brushTool" title="Brush Tool (B)">🖌</div>
    </div>
    <div class="tool-group">
      <div class="tool-btn" id="rectangleTool" title="Rectangle Tool (M)">□</div>
      <div class="tool-btn" id="ellipseTool" title="Ellipse Tool (L)">○</div>
      <div class="tool-btn" id="polygonTool" title="Polygon Tool">⬡</div>
      <div class="tool-btn" id="starTool" title="Star Tool">★</div>
      <div class="tool-btn" id="lineTool" title="Line Tool (/)">╱</div>
    </div>
    <div class="tool-group">
      <div class="tool-btn" id="typeTool" title="Type Tool (T)">T</div>
      <div class="tool-btn" id="typeOnPathTool" title="Type on Path Tool">↝</div>
      <div class="tool-btn" id="touchTypeTool" title="Touch Type Tool">𝓣</div>
    </div>
    <div class="tool-group">
      <div class="tool-btn" id="shapeBuilderTool" title="Shape Builder Tool (Shift+M)">⧆</div>
      <div class="tool-btn" id="perspectiveGridTool" title="Perspective Grid Tool">▦</div>
      <div class="tool-btn" id="freeTransformTool" title="Free Transform Tool (E)">⤡</div>
    </div>
    <div class="tool-group">
      <div class="tool-btn" id="eyedropperTool" title="Eyedropper Tool (I)">💧</div>
      <div class="tool-btn" id="measureTool" title="Measure Tool">📏</div>
    </div>
    <div class="tool-group">
      <div class="tool-btn" id="handTool" title="Hand Tool (H)">✋</div>
      <div class="tool-btn" id="zoomTool" title="Zoom Tool (Z)">🔍</div>
    </div>
    <div class="tool-group">
    ...
    <div class="panel-content">
      ...
      <div id="panel-align" class="panel-section">
        ...
        <div class="align-grid">
          <button class="align-btn" onclick="align('left')" title="Align Left">⬅</button>
          <button class="align-btn" onclick="align('center-h')" title="Align Center">⬌</button>
          <button class="align-btn" onclick="align('right')" title="Align Right">➡</button>
          <button class="align-btn" onclick="align('top')" title="Align Top">⬆</button>
          <button class="align-btn" onclick="align('center-v')" title="Align Center">⬍</button>
          <button class="align-btn" onclick="align('bottom')" title="Align Bottom">⬇</button>
        </div>
        ...
        <div class="align-grid">
          ...
          <button class="align-btn" onclick="distribute('right')">⫹</button>
          <button class="align-btn" onclick="distribute('top')">⬏</button>
          <button class="align-btn" onclick="distribute('center-v')">⬎</button>
          <button class="align-btn" onclick="distribute('bottom')">⬐</button>
        </div>
      </div>
      ...
      <div id="panel-character" class="panel-section">
        ...
        <div class="btn-group">
          <button class="btn" onclick="setTextAlign('left')">⬅</button>
          <button class="btn" onclick="setTextAlign('center')">⬌</button>
          <button class="btn" onclick="setTextAlign('right')">➡</button>
          <button class="btn" onclick="setTextAlign('justify')">≡</button>
        </div>
      </div>
...
<div class="context-menu" id="contextMenu">
  ...
  <hr>
  <div onclick="groupSelection()">Group</div>
  <div onclick="ungroupSelection()">Ungroup</div>
  <hr>
  <div onclick="deleteSelection()">Delete</div>
</div>
...
var tools = {};
var currentTool;
var fillColor = new Color(1, 0, 0); // RGBA
var strokeColor = new Color(0, 0, 0); // RGBA
var strokeWidth = 1;
var currentPath;
var isDrawing = false;
var startPoint;
...
var artboardSize = { width: 595, height: 842 }; // A4 size in points (72 DPI)
...
// Selection Tool with dashed selection box
...
tools.select.onMouseDrag = function(event) {
  if (selectionStart) {
    // Show dashed selection box
    var rect = new Rectangle(selectionStart, event.point);
    var canvasRect = view.element.getBoundingClientRect();
    
    selectionBox.style.display = 'block';
    selectionBox.style.left = (rect.point.x + 60) + 'px';
    selectionBox.style.top = (rect.point.y + 20) + 'px';
    selectionBox.style.width = rect.width + 'px';
    selectionBox.style.height = rect.height + 'px';
    
    // Select items within the box
    project.deselectAll();
    var bounds = new Rectangle(selectionStart, event.point);
    project.activeLayer.children.forEach(function(item) {
      if (item.name !== 'Artboard' && !item.locked && item.bounds.intersects(bounds)) {
        item.selected = true;
      }
    });
  } else if (project.selectedItems.length) {
...
// Pen Tool with bezier curve support (Illustrator-style)
tools.pen = new Tool();
var penPoints = [];
...
tools.pen.onMouseDrag = function(event) {
  if (currentPath && penPoints.length > 0) {
    // Create bezier curve by dragging - Illustrator style
...
// Type Tool with resizable text box
tools.type = new Tool();
...
tools.type.onMouseDown = function(event) {
  ...
  typeCurrentShape.strokeColor = '#60a5fa';
...
tools.type.onMouseUp = function(event) {
  if (typeCurrentShape) {
    ...
    if (text) {
      ...
      areaText.fontSize = Math.min(bounds.width, bounds.height) * 0.5;
      areaText.fontFamily = 'Arial';
      areaText.justification = 'center';
...
tools.typeOnPath.onMouseDown = function(event) {
  ...
  if (hit && hit.item instanceof Path) {
    ...
    if (text) {
      ...
      pathText.fontSize = 24;
      pathText.fontFamily = 'Arial';
  ...
  for (var i = layers.length - 1; i >= 0; i--) {
    ...
    layerItem.innerHTML = `
      ...
      <div class="layer-actions">
        <button class="btn btn-sm" onclick="toggleLayerVisibility(${i})">${layer.visible ? '👁' : '👁‍🗨'}</button>
        <button class="btn btn-sm" onclick="toggleLayerLock(${i})">${layer.locked ? '🔒' : '🔓'}</button>
      </div>
    `;
...
function groupSelection() {
  if (project.selectedItems.length > 1) {
    ...
    alert('Select 2 or more objects to group');
  }
}
