...
class MultiplayerManager {
    ...
    toggleReady() {
        if (this.room) {
            try {
                ...
                console.warn('Error sending ready state:', error);
            }
        }
    }
    ...
    sendScoreUpdate(score, combo, health) {
        if (this.room) {
            try {
                ...
                console.warn('Error sending score update:', error);
            }
        }
    }
    ...
    sendGameFinished(finalScore) {
        if (this.room) {
            try {
                ...
                console.warn('Error sending game finished:', error);
            }
        }
    }
    ...
    async connectToServer() {
        if (!this.client) {
            ...
            // Direct connection to ngrok server
            const serverUrl = 'https://filtratable-lophodont-temeka.ngrok-free.dev';
    ...
    handleRoomLeave() {
        ...
        // Only cleanup if room still exists (prevent double leave)
        if (this.room) {
            this.room = null;
            this.lobbyCode = null;
            this.isHost = false;
