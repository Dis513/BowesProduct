...
class MultiplayerManager {
    ...
    async createRoom() {
        try {
            ...
            if (roomType === 'private') {
                ...
                // Public rooms don't show/have codes
            ...
            this.showRoomInfo(roomCode, roomType);
    ...
    async joinRoom(code) {
        try {
            ...
            this.room = await this.client.joinById('rhythm_game', {
                roomCode: code,
                isHost: false
            });
            ...
            this.showRoomInfo(code, 'private');
            this.updateConnectionStatus(true);
            ...
            if (roomType === 'public' || playerCount < maxPlayers) {
                ...
                lobbyItem.innerHTML = `
                    <div class="lobby-info">
                        <span class="lobby-code">${roomType === 'public' ? '🌍 Public' : '🔒 ' + roomCode}</span>
                        <span class="lobby-players">${playerCount}/${maxPlayers} players</span>
                    </div>
                    <button class="join-lobby-btn" data-code="${roomCode}">
                        Join
                    </button>
                `;
