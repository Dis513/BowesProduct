...
class MultiplayerManager {
        ...
        const multiplayerHTML = `
            ...
            <div class="menu-container" id="joinRoomMenu" style="display: none;">
                ...
                <div class="room-joining">
                    <label>Enter Room Code (Private Rooms):</label>
                    <input type="text" id="joinRoomCode" class="themed-input" placeholder="Enter code for private room...">
                    <button class="menu-button" id="confirmJoinRoom">Join with Code</button>
                </div>
                
                <div class="lobby-list-section">
                    <label class="section-divider">— OR —</label>
                    <label>Available Lobbies:</label>
                    <button class="menu-button" id="refreshLobbies">🔄 Refresh Lobbies</button>
                    <div class="lobby-list" id="lobbyList">
                        <div class="lobby-empty">Loading available lobbies...</div>
                    </div>
                </div>
    ...
    setupEventListeners() {
        ...
        // Refresh lobbies
        const refreshLobbiesBtn = document.getElementById('refreshLobbies');
        if (refreshLobbiesBtn) {
            refreshLobbiesBtn.addEventListener('click', () => this.refreshLobbyList());
        }
    ...
    showMultiplayerMenu() {
        ...
        // Reset opponent status when showing multiplayer menu
        if (this.waitingRoom) {
    ...
    hideMultiplayerMenu() {
        // Reset opponent status when hiding multiplayer menu
        if (this.waitingRoom) {
            this.updateWaitingRoom(false);
        }
    ...
    showCreateRoom() {
        ...
        // Reset opponent status when opening create room
        if (this.waitingRoom) {
            this.updateWaitingRoom(false);
        }
        ...
        if (this.createRoomMenu) {
            ...
            console.error('createRoomMenu element not found!');
        }
    }

    hideCreateRoom() {
        // Reset opponent status when canceling create room
        if (this.waitingRoom) {
            this.updateWaitingRoom(false);
        }
    ...
    showJoinRoom() {
        // Reset opponent status when opening join room
        if (this.waitingRoom) {
            this.updateWaitingRoom(false);
        }
        ...
        if (this.joinRoomMenu) {
            ...
            // Auto-refresh lobby list when opening join room
            this.refreshLobbyList();
        }
    }

    hideJoinRoom() {
        // Reset opponent status when canceling join room
        if (this.waitingRoom) {
            this.updateWaitingRoom(false);
        }
    ...
    async refreshLobbyList() {
        ...
        lobbyList.innerHTML = '<div class="lobby-empty">Loading available lobbies...</div>';

        try {
            const rooms = await this.fetchAvailableLobbies();
            
            if (rooms.length === 0) {
                lobbyList.innerHTML = '<div class="lobby-empty">No available lobbies found. Create a room to start!</div>';
                return;
            }
            ...
            rooms.forEach(room => {
                ...
                const isPrivate = roomMetadata.private || false;
                const songId = roomMetadata.songId || 1;
                const playerCount = room.clients || 0;
                const maxClients = room.maxClients || 2;
                
                // Only show non-private rooms in the list
                if (!isPrivate) {
                    const lobbyItem = document.createElement('div');
                    lobbyItem.className = 'lobby-item';
                    lobbyItem.innerHTML = `
                        <div class="lobby-info">
                            <span class="lobby-song">Level ${songId}</span>
                            <span class="lobby-players">${playerCount}/${maxClients} Players</span>
                        </div>
                        <button class="lobby-join-btn" data-room-id="${room.roomId}">Join</button>
                    `;
                    
                    // Add click handler for join button
                    const joinBtn = lobbyItem.querySelector('.lobby-join-btn');
                    joinBtn.addEventListener('click', () => this.joinRoomById(room.roomId));
                    
                    lobbyList.appendChild(lobbyItem);
                }
            });

            // If no public rooms found
            if (lobbyList.children.length === 0) {
                lobbyList.innerHTML = '<div class="lobby-empty">No public lobbies available. Create a room to start!</div>';
            }
            
        } catch (error) {
            console.error('Failed to refresh lobby list:', error);
            lobbyList.innerHTML = '<div class="lobby-empty">Failed to load lobbies. Please try again.</div>';
        }
    }
    ...
    async createRoom() {
        try {
            ...
            console.log('Room options:', { songId, isPrivate });
            ...
            // Create a new room (not join existing)
            console.log('Calling client.create...');
            this.room = await this.client.create('rhythm_game', options);
            ...
            if (errorMsg.includes('timeout') || errorMsg.includes('TIMEOUT')) {
                this.showError('Connection timed out. The multiplayer server may be down or experiencing issues. Please try again later.');
            } else if (errorMsg.includes('full') || errorMsg.includes('maxClients')) {
                this.showError('No available rooms. Please try again or create a new room.');
            } else if (errorMsg.includes('connection') || errorMsg.includes('ERR_CONNECTION')) {
                this.showError('Cannot connect to multiplayer server. Please check your internet connection and try again.');
            } else {
                this.showError(`Failed to create room: ${errorMsg}. Please try again.`);
            }
        }
    }
    ...
    async joinRoom() {
        try {
            ...
            if (errorMsg.includes('timeout') || errorMsg.includes('TIMEOUT')) {
                ...
                this.showError('This room is full. Please try a different room code.');
            } else if (errorMsg.includes('not found') || errorMsg.includes('invalid')) {
                this.showError('Invalid room code. Please check and try again.');
            } else if (errorMsg.includes('connection') || errorMsg.includes('ERR_CONNECTION')) {
                this.showError('Cannot connect to multiplayer server. Please check your internet connection and try again.');
            } else {
                this.showError(`Failed to join room: ${errorMsg}. Please try again.`);
            }
        }
    }
    ...
    async joinRoomById(roomId) {
        try {
            ...
            this.showError('Failed to join room. It may be full or no longer available.');
        }
    }

    async fetchAvailableLobbies() {
        try {
            if (!this.client) {
                await this.initialize();
            }
            ...
            if (errorMsg.includes('timeout') || errorMsg.includes('TIMEOUT')) {
                console.warn('Lobby fetch timeout - server may be unavailable');
            } else if (errorMsg.includes('connection') || errorMsg.includes('ERR_CONNECTION')) {
                console.warn('Cannot connect to server for lobby list');
            }
    ...
    setupRoomHandlers() {
        ...
        this.room.onMessage('playerJoined', (data) => {
            ...
            // Only update UI if the opponent joined (not yourself)
            if (data.playerId !== this.room.sessionId && this.waitingRoom) {
        ...
        this.room.onMessage('playerLeft', (data) => {
            ...
            // Reset opponent status when player leaves
            if (this.waitingRoom) {
                this.updateWaitingRoom(false);
            }
    ...
    showWaitingRoom(songId) {
        ...
        if (this.waitingRoom) {
            ...
            // Reset opponent status when showing waiting room
            this.updateWaitingRoom(false);
        }
    }
    ...
    showQuickMatchWaiting() {
        ...
        const waitingHTML = `
            <div class="menu-container" id="quickMatchWaiting" style="display: flex;">
    ...
    showMultiplayerResult(playerWon, playerScore, opponentScore) {
        const resultHTML = `
            <div class="menu-container" id="multiplayerResult" style="display: flex;">
        ...
        setTimeout(() => {
            ...
            if (playAgainBtn) {
                playAgainBtn.addEventListener('click', () => {
                    const resultModal = document.getElementById('multiplayerResult');
    ...
    handleOpponentLeft() {
        ...
        const opponentLeftHTML = `
            <div class="menu-container" id="opponentLeftModal" style="display: flex;">
    ...
    async leaveRoom() {
        ...
        // Hide quick match waiting if visible
        const quickMatchWaiting = document.getElementById('quickMatchWaiting');
        if (quickMatchWaiting) {
            quickMatchWaiting.remove();
        }
        ...
        // Reset opponent status before leaving
        if (this.waitingRoom) {
            this.updateWaitingRoom(false);
        }
        ...
        const menus = ['waitingRoom', 'createRoomMenu', 'joinRoomMenu'];
        menus.forEach(id => {
            const menu = document.getElementById(id);
            if (menu) menu.style.display = 'none';
        });
